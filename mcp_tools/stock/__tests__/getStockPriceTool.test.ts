...
const CTX = { sessionId: "test-session", tenantId: "agent-1" };
...
test("daily bars are condensed rather than passed through", async () => {
  ...
  const result = await tool.execute({ symbol: "AAPL" }, { sessionId: "s", tenantId: "agent-1" });
...
test("historyDays is clamped and the clamp is reported", async () => {
  ...
  const result = await tool.execute({ symbol: "AAPL", historyDays: 99_999 }, { sessionId: "s", tenantId: "agent-1" });
  const data = result.generation_context!.data as Record<string, unknown>;
...
test("a call with no historyDays asks for a year", async () => {
  ...
  await tool.execute({ symbol: "AAPL" }, { sessionId: "s", tenantId: "agent-1" });
...
test("a historyDays within the limit produces no note", async () => {
  ...
  const result = await tool.execute({ symbol: "AAPL", historyDays: 1260 }, { sessionId: "s", tenantId: "agent-1" });
...
test("a fractional historyDays floors to at least 1 day, not 0", async () => {
  ...
  await tool.execute({ symbol: "AAPL", historyDays: 0.5 }, { sessionId: "s", tenantId: "agent-1" });
