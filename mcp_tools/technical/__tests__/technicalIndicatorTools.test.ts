...
test("an indicator reads the requested stock bars from the repository", async () => {
  ...
  const result = await tool.execute(
    ...
    { sessionId: "test", tenantId: "agent-1" },
  );
...
test("every indicator returns structured stock data without a prompt template", async () => {
  ...
  for (const tool of createTechnicalIndicatorTools({ getRepository: async () => repository })) {
    const result = await tool.execute({ symbol: "SPY" }, { sessionId: "test", tenantId: "agent-1" });
...
test("reports the database as unavailable only when the direct fetch also fails", async () => {
  ...
  const result = await tool.execute({ symbol: "AAPL" }, { sessionId: "test", tenantId: "agent-1" });
...
test("rejects a missing symbol before opening the stock repository", async () => {
  ...
  const result = await tool.execute({}, { sessionId: "test", tenantId: "agent-1" });
