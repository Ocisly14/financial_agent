...
import { INSTITUTIONAL_ADOPTION_PROMPT_TEMPLATE } from "./prompts.ts";
...
export function createInstitutionalAdoptionSearchTool(): RegisteredTool {
  return {
    name: "institutional_adoption_search",
    description:
      "Search for institutional cryptocurrency adoption signals including ETFs, treasury holdings, custody, funds, filings, and corporate adoption.",
    ...
    inputSchema: {
      ...
      properties: {
        task: {
          type: "string",
          description: "Natural-language task or question about institutional crypto adoption.",
        },
        query: {
          type: "string",
    ...
    execute: async (input: JsonObject): Promise<ToolExecutionResult> => {
      ...
      const query =
        `${baseQuery} institutional adoption ETF treasury custody regulatory`;
      ...
      try {
        ...
        const prompt =
          results.length > 0
            ? `${INSTITUTIONAL_ADOPTION_PROMPT_TEMPLATE}\n\nSearch results:\n${resultList}`
        ...
        return {
          summary: `Institutional adoption search returned ${results.length} results.`,
          generation_context: { prompt, data },
        };
      } catch (err: unknown) {
        const message = err instanceof Error ? err.message : String(err);
        return {
          summary: `Institutional adoption search failed: ${message}`,
          generation_context: {
            prompt: INSTITUTIONAL_ADOPTION_PROMPT_TEMPLATE,
            data: { query, symbol, result_count: 0, results: [], error: message },
          },
        };
      }
    },
