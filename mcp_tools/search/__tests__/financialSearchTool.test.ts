...
test("requires an explicit query and does not call Tavily without one", async () => {
  ...
  const result = await tool.execute({ task: "find bank earnings" }, { sessionId: "test", tenantId: "agent-1" });
...
test("uses generic defaults and clamps the result limit", async () => {
  ...
  await tool.execute({ query: "global bond market outlook", limit: 100 }, { sessionId: "test", tenantId: "agent-1" });
