...
服务跑在 :3100（避开 :3000 上的既有进程），LLM provider = google，新建 topic `room_density_1785779770741`，六个对照提问顺序提出。

### 12.1 模型选择的参数 —— 主要风险已解除

§10.2 的风险是：模型的旧习惯是"调大 `historyDays` 自己找"，clamp 是硬约束但 description 只是软引导。实测六轮里 `get_stock_price` 的全部调用：

```
turn 1: {"symbol":"AAPL"}
turn 3: {"symbol":"NVDA","historyDays":250}
turn 4: {"symbol":"NVDA","window":{"from":"2026-01-30","to":"2026-01-30"}}
turn 5: {"symbol":"AAPL","window":{"from":"2026-01-01","to":"2026-06-30"}}
turn 6: {"symbol":"AAPL","window":{"from":"2026-01-01","to":"2026-08-03"}}
turn 6: {"symbol":"NVDA","window":{"from":"2026-01-01","to":"2026-08-03"}}
```

**没有一次超量 `historyDays`。** 问到具体日期时模型选了 `window`，而且第 4 题用的是**单日区间** `2026-01-30..2026-01-30` —— 正是 §5.0 那条 173KB 记录想表达却表达不出来的东西。问"过去一年"时用 `historyDays: 250`（新默认），锚点选择正确。

第 4 题的答案 `$190.90` 与改动前那条 1000 根 bar 的记录得出的数值完全一致，且 `window.recentBars[0]` 是精确的单根 bar（`c: 190.9`）。

### 12.2 体积

| | 改前 | 改后 |
|---|---|---|
| 最大 `get_stock_price` task_result | 173,266 | **11,299** |
| 单 symbol 行情载荷 | ~84,700 | ~3,100 |

（本次最大的 task_result 是 20,440 字节，来自 turn 2 的 `financial_search` 网页结果，与行情无关。）

`condenseBars` 的离线实测：

| 场景 | 原始 | 压缩后 | 倍数 |
|---|---|---|---|
| daily 1000 根 | 86,448 | 3,142 | 27.5x |
| intraday 960 根 | 82,981 | 2,814 | 29.5x |
| window 150 根 | 12,984 | 5,310 | 2.4x（`bucketDays=1`） |

### 12.3 答案准确性

六题全部正常作答，无"数据不足"式退化。抽查 turn 3 的每个数字，全部来自 `daily.stats` 且逐位一致：`returnPct 16.98`、`maxDrawdownPct 20.21`、`sma20 203.95`、`sma50 205.81`、极值 `164.99@2026-03-30` / `235.51@2026-05-14`。模型没有自行重算，也没有编造。

### 12.4 `stats` 极值的措辞 —— 已发现并修复

首轮回归里模型把 `stats.max` / `stats.min` 标成了 **"52-week high" / "52-week low"**。数字来源正确，标签错误：那是**收盘价**极值。

提示文本当时已经写明 `min/max are the closing low and closing high`，模型仍然没照做。因此没有继续加禁令，而是把它真正想要的字段给它。

**改动**：`computeStats` 增加 `trueLow` / `trueHigh`，取 `l` / `h` 的极值并带首次出现日期。

这不只是措辞问题。**降采样之后真实极值原本不可恢复**：`recentBars` 只有尾部 7 根带 h/l，`trend` 只有收盘价，一根发生在中段的插针在 digest 里完全看不见。所以这两个字段补的是压缩造成的真实信息损失。`barDigest.test.ts` 有一个用例专门锁这点：第 25 根 bar 构造 `h:180 / l:40 / c:100` 的插针，断言 `trueHigh === 180`、`max === 100`，且该根不在 `recentBars` 内。

**复测（2026-08-03，topic `room_extremes_1785784148`）**：

| | 模型报的 | `trueHigh`/`trueLow` | `max`/`min`（收盘） |
|---|---|---|---|
| NVDA 52周最高 | 236.26 @2026-05-14 | 236.26 @2026-05-14 | 235.51 @2026-05-14 |
| NVDA 52周最低 | 163.86 @2025-09-05 | 163.86 @2025-09-05 | 164.99 @**2026-03-30** |
| AAPL 52周最高 | 344.56 @2026-07-29 | 344.56 @2026-07-29 | 340.01 @**2026-07-28** |
| AAPL 52周最低 | 201.37 @2025-08-05 | 201.37 @2025-08-05 | 202.13 @2025-08-05 |

模型改口了。注意 NVDA 的低点不只是差 1.13 元——**日期完全是另一天**，收盘最低与真实最低发生在不同交易日。这是原标签错误的实际后果。

**代价**：每个 digest +89 字节。1000 根日线 3,142 → 3,231，压缩比 27.5x → 26.8x。
