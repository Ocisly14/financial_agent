...
test("putBars dedupes and returns ascending by date", async () => {
  ...
  const bars = await store.getBars("AAPL", TF, 10);
...
test("symbols don't interfere with each other", async () => {
  ...
  assert.equal((await store.getBars("AAPL", TF, 10))[0]!.c, 1);
...
test("clearSymbol clears that symbol's bars and coverage", async () => {
  ...
  await store.clearSymbol("AAPL", TF);
  assert.deepEqual(await store.getBars("AAPL", TF, 10), []);
...
test("coverage can be written and read back", async () => {
  ...
  assert.equal((await store.getCoverage("AAPL", TF))?.lastDate, "2026-07-27");
  assert.equal(await store.getCoverage("MSFT", TF), undefined);
});
