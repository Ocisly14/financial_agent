...
function replyingWith(text: string): { router: ModelRouter; calls: () => number; lastOptions: () => GenerateOptions | undefined } {
  let calls = 0;
  let lastOptions: GenerateOptions | undefined;
  const provider: LlmProvider = {
    name: "fake",
    async generate(_messages: LlmMessage[], options: GenerateOptions) {
      calls++;
      lastOptions = options;
      return { text, metrics: { tokens_in: 0, tokens_out: 0, ms: 0, model_class: options.modelClass, provider: "fake" } };
    },
  };
  return { router: new ModelRouter(provider), calls: () => calls, lastOptions: () => lastOptions };
}
...
test("an empty history spends no model call", async () => {
  ...
  assert.deepEqual(await generateDigest([], router), { summary: "", category: null });
...
test("JSON wrapped in prose or a fence is still read", () => {
  ...
  assert.deepEqual(fenced, { summary: "半导体周期", category: "sector" });

  const chatty = parseDigestReply('Sure! Here you go:\n{"summary": "配对交易", "category": "comparative"}\nHope that helps.');
  assert.deepEqual(chatty, { summary: "配对交易", category: "comparative" });
});
