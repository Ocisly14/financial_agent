...
// DerivationDeps.treasuryRiskFree, so these tests never touch the network. Other terms are left unreachable
...
function baseInput(treasuryRiskFree?: (asOfDate: string) => Promise<{ value: number; curveDate: string } | undefined>) {
  return {
    symbol: "AAPL",
    asOfDate: ASOF,
    facts: [],
    ...
    deps: {
      dailyCloses: async () => [],
      ...(treasuryRiskFree ? { treasuryRiskFree } : {}),
    },
  };
}
...
test("riskFreeRate derives as a market-sourced term when the Treasury dep resolves", async () => {
  ...
  assert.deepEqual(rf!.derivation, { term: "10Y", curveDate: "2026-01-02", feed: "treasury.gov" });
...
test("riskFreeRate is unreachable, with a clear reason, when the Treasury dep resolves to undefined", async () => {
  ...
  assert.ok(entry);
  assert.equal(entry!.reason, "treasury.gov 10Y yield unavailable; supply it as an override");
});

test("riskFreeRate is unreachable when no treasuryRiskFree dep is wired at all", async () => {
