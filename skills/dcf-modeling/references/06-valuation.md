...
The equity bridge demands a numeric value at the anchor for every bridge row (cash available, non-operating investments, debt, leases, preferred, NCI). A row the issuer simply lacks must be handled explicitly, not left null: construct it from mapped rows where honest — `cash_available_for_bridge` over the historical range, as `cash_and_equivalents + short_term_investments` — and zero only a genuinely absent obligation, with a rationale citing that absence. The host derives the formula source from the write. `incomplete_equity_bridge` names exactly which row is missing.
