...
tools: [extract_filing_statements, create_financial_model, run_dcf_subagent, apply_financial_model_operations, get_financial_model, list_financial_models, archive_financial_model, list_unified_statements, get_unified_rows, calculate_model_rows, get_treasury_yield, financial_search, read_skill_reference]
---

# DCF valuation

A DCF is one long-running financial_modeling job, not a conversation. Dispatch the agent with the ticker and let it work stepwise; it pauses resumably at its step limit, and every summary carries the model_id — dispatch again with that model_id to continue where it stopped. Expect several rounds: data foundation first, then forecast authoring, then valuation. It is done when the model reaches the valued stage and the agent reports the intrinsic value with its sensitivity table and the load-bearing assumptions. Relay those assumptions with the number — a DCF without its assumptions is not a result.
