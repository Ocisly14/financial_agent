...
- **One shared story** → the default margin-driven chain is honest: set the six fixed drivers' forecast assumptions (growth.revenue.total, margin.operating, tax_rate, ratio.da_to_revenue, ratio.capex_to_revenue, ratio.operating_nwc_to_revenue) against their anchors and move on. Keeping the default chain is itself a Move-2 judgment — it asserts every source shares one persisting story, and needs that justification like any other claim.
- **Different stories per source** (a shrinking segment beside an exploding one) → the single margin cannot say that. Rewrite: give each revenue stream its growth assumption (streams already carry `LAG(stream,1) * (1 + growth.revenue.<stream>)` forecast formulas), switch revenue.total to formula-sourced and make it the sum of streams; drive costs at whatever level your story actually lives at — toolbox §3 has the exact operation sequences, the set_line_item_source prerequisites, and the fade patterns (YEAR_INDEX) for stories that are neither "changes now" nor "never changes".
